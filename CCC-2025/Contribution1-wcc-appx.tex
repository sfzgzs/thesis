...
\begin{frame}{Result 1 - Background - \WhileCC\ Programming Language}
    ...
    \begin{minipage}[t]{0.67\linewidth}
        ...
        \begin{table}[]
            ...
            \begin{tabular}{|c|c|}
                ...
                $n := \CHOOSE\ (k : \nat) : k < 0$ & $\{\uparrow \}$\\
                \hline
                \pause $n := \CHOOSE\ (k : \nat) : \mathsf{toReal}(k) = 0$ & $\{\uparrow \}$\\
                \hline
                \pause $n := \CHOOSE\ (k : \nat) : k < k + 1$ & $\{0,1,2,\cdots\}$ \\
                \hline
                \pause $n := \CHOOSE (k : \nat) : k > 2\ \mathsf{and}\ k<4$ & $\{3 \}$\\
                \hline
            \end{tabular}
    ...
    We are using here have been designed to make all expressed functions continuous on their domains ,
...
\label{def::whileCC-approximable}
    ...
    \begin{itemize}
        \pause \item $x \in \dom(f) \then \uparrow \notin P^\algebraR(x,n) $ \pause  $\subseteq \Nbd(f(x), 2^{-n})$
            , and
        \pause \item $x \notin \dom(f) \then P^\algebraR(x, n)=\{ \uparrow\} $
    \end{itemize}
    \pause where $\Nbd(y,r)$ has the standard definition of neighborhood on $\Reals$ i.e., \[\Nbd(y,r) = \{ z\in \Reals\ \mid \abs{y-z}<r \}.\]
\end{definition}

     \pause \begin{exampleblock}{\textbf{\textcolor{OliveGreen}{Goal}}}
         Construct \WhileCC-procedures approximating elementary functions by induction
     \end{exampleblock}
     
     \pause \begin{exampleblock}{\textcolor{BrickRed}{\textbf{Challenge:} Using comparison operators introduces undefinedness.}}
